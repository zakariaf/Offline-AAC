...
> Phase: **author** · Agent `ad36b311fcd342abb` · Run `wf_12b14467-451`
...
### CRITIQUE 1
# COMPLETENESS CRITIC — Offline-AAC engineering corpus

I verified the contested items against the SDK on this machine (Dart 3.11.0 / Flutter 3.41.2), the resolved pub cache, the Flutter SDK source, and the live `subosito/flutter-action` parser. Findings below are marked **[VERIFIED]** where I ran or read the thing, **[OPEN]** where I did not.

---

## 1. CONTRADICTIONS BETWEEN DIMENSIONS

### 1.1 THE BIG ONE: three incompatible `speak()` signatures, all shipped as verbatim copy-paste code

This is the corpus's worst defect and no fact-check caught it, because each dimension is internally consistent.

| Dimension | Signature | Failure model |
|---|---|---|
| official-architecture | `Future<Result<void>> speak(String)` | `Result.error(Exception)` |
| dart3-idioms | `Future<SpeakOutcome> speak(String)` | sealed `SpeakFailure` carrying `spokenText` |
| platform-channel-testing | `Future<SpeakResult> speak(String)` | `enum SpeakFailure` + `onFailure` callback |

All three are presented as "the" `SpeechService`. Worse, **testing-strategy's** `FakeSpeechService` implements a *fourth* contract (`Future<void> speak` that **throws**) — which directly contradicts dart3-idioms' central thesis that speak must never throw for expected failures. And **widget-golden-testing's** fake implements a *fifth* (`Future<void>` + `failWith` throwing).

**Resolution (evidence-based).** dart3-idioms wins on the merits and its fact-check is why:
- **[VERIFIED]** Non-exhaustive switch on a sealed type is `compileTimeError` (`non_exhaustive_switch_statement`), not a lint. That's real leverage.
- **[VERIFIED]** flutter_tts returns `result.success(0)`; it does not throw. So testing-strategy's throwing fake models a failure mode **the plugin cannot produce**, and its "silence is impossible" test — the one it calls "the highest-value single test in the app" — is therefore testing fiction. That test must be rewritten against a *status-code* fake.
- official-architecture's `Result<T>` error arm is typed `Exception` → zero exhaustiveness → cannot force handling of a *new* failure mode, which is the entire stated goal.

Adopt: `Future<SpeakOutcome> speak(String)` with payload-carrying sealed failures. Keep `Result<T>` (renamed `Ok`/`Err`) **only** for the drift repository. Every fake in the corpus must be rewritten to that one contract. **Ship exactly one `SpeechService` definition in the docs.**

### 1.2 `close_sinks` — the shipped config contains a no-op safety rule **[VERIFIED, I ran it]**

The repo's `analysis_options.yaml:92` sets `close_sinks: error` with a comment saying it overrides VGV "on purpose" for the TTS voice-availability `StreamController`. **It does nothing.**

- **[VERIFIED]** VGA 10.2.0 contains exactly one `close_sinks` occurrence: `close_sinks: ignore` at line 8, inside `analyzer: errors:`. There is **no** `- close_sinks` under `linter: rules:` — the rule is never enabled. `analyzer: errors:` only re-ranks diagnostics that are *already generated*.
- **[VERIFIED]** Probe with the shipped file verbatim + a never-closed `StreamController`: **no diagnostic**. Meanwhile `empty_catches` and `avoid_catches_without_on_clauses` — promoted in the *same* `errors:` block — fired as errors. So 16 of the 17 promotions work and this one is dead.

**But the fact-check is also wrong.** It claims that even properly enabled, `close_sinks` "will not cover the field-held controller." My probe:

```
error • Unclosed instance of 'Sink' • lib/main.dart:5:9  • close_sinks   <- local, never closed
error • Unclosed instance of 'Sink' • lib/main.dart:19:34 • close_sinks   <- FIELD-held, never closed
(line 13: field-held, closed in dispose() -> correctly SILENT, no false positive)
```

Field-held-and-never-closed **fires**. Field-held-and-closed-in-`dispose()` correctly stays silent. That is exactly the desired behavior for the described design. **Fix:** add a `linter: rules:` block with `- close_sinks`, keep `errors: close_sinks: error` to promote info→error. Both the claim and its refutation were wrong; the rule is worth having and is currently absent.

### 1.3 The shipped `plugins:` block likely enables two dead riverpod rules — and the riverpod dimension contradicts *itself*

`analysis_options.yaml:170-177` enables `avoid_build_context_in_providers: true` and `provider_dependencies: true`. The riverpod dimension explicitly lists **both** as generator-only (`@riverpod`), and the project skips codegen. If that classification is right, these two lines are dead config. It also *omits* `avoid_public_notifier_properties` and `async_value_nullable_pattern`, which that same dimension says **do** work without codegen.

Worse, the riverpod dimension's flagship MediaQuery recommendation justifies itself with *"which riverpod_lint bans via `avoid_build_context_in_providers`"* — a rule it elsewhere classifies as generator-only, i.e. **unenforced in this project**. The MediaQuery conclusion is still right (it's right on Flutter-architecture grounds), but its stated enforcement mechanism doesn't exist. **[OPEN — the one config claim I did not empirically confirm; it needs a probe with a hand-written provider reading BuildContext before shipping.]**

### 1.4 `pumpAndSettle` — the corpus violates its own proposed CI gate

testing-strategy: *"Ban pumpAndSettle. Add a lint or grep-based CI check that fails on the string 'pumpAndSettle'."* That grep would fail on **the corpus's own code examples**: widget-golden-testing's drift boot test (`await tester.pumpAndSettle();`) and riverpod's widget test both call it. Two dimensions ship code the third would reject in CI.

**Resolution:** the fact-check is right that `pump()` is not always sufficient — it doesn't advance the fake clock, and drift/TTS futures need `pump(duration)`. The honest rule is: **ban `pumpAndSettle` as an *animation* wait; use `pump()` for state and `pump(duration)` for time-based async.** Fix the two examples before they become the template.

### 1.5 Coverage floor vs. what is actually testable

ci-release's script gates `lib/speech/` at **100%**. But `lib/speech/` contains `flutter_tts_speech_service.dart` — the platform wrapper — and platform-channel-testing says confine channel mocking to **one contract-test file**. You cannot hit 100% on a directory containing an untestable-without-channel-mocks plugin wrapper while also confining channel mocks. The two recommendations are jointly unsatisfiable.

**Resolution:** the 100% floor belongs on **files**, not directories: `voice_filter.dart` (pure — testing-strategy's own examples prove it's 100%-able), `crash_log.dart`, `board_repository.dart`, `migrations.dart`. The plugin wrapper gets the one contract test and no floor. Fix ci-release's `FLOORS` array from prefixes to file paths.

### 1.6 `lib/utils/` — mandated and forbidden

official-architecture's tree mandates `lib/utils/result.dart` + `lib/utils/command.dart`. project-structure's tree explicitly **cuts** `lib/utils/` as a "junk drawer" and ships no `Result` at all. Trivially resolvable, but the two trees are presented as authoritative and are not reconcilable as written. Given §1.1, put the sealed outcomes next to the service they describe and drop `lib/utils/` and `command.dart` (Command's `if (_running) return` guard has no valid use once speak is barge-in and edit-mode writes are single-digit-ms local SQLite).

### 1.7 flutter_lints vs very_good_analysis

project-structure ships an `analysis_options.yaml` based on `flutter_lints`; lints-tooling ships one based on VGA, empirically probed. **[VERIFIED]** VGA 10.2.0 and riverpod_lint 3.1.3 resolve cleanly on this Dart 3.11 machine. lints-tooling wins — it ran the file. Delete project-structure's competing config from the docs entirely; two analysis_options in one corpus is how a stranger ends up with the weaker one.

### 1.8 Riverpod: two claims are dead **[VERIFIED]**

- *"Notifiers are recreated on every provider rebuild → holding a TTS controller as a Notifier field leaks"* — **REFUTED.** CHANGELOG line 139, under `3.0.0-dev.16 (2025-06-20)`: *"Revert Notifier life-cycle change. They are once again preserved across rebuilds."* Stable is 3.0.0 (2025-09-10). The `must`-level recommendation built on this is reasoning from a false model. (The recommendation itself — services in a plain `Provider` with `ref.onDispose` — is still fine, just not *forced*.) `Ref.mounted` does exist (`ref.dart:112`).
- *"The `.autoDispose` modifier and all AutoDispose\* classes are gone"* — **REFUTED.** `providers/provider.dart:49`: `static const autoDispose = AutoDisposeProviderBuilder();`, plus `stream_provider.dart:121`, `notifier/orphan.dart:111`. Only the *interface clones* (`AutoDisposeRef`/`AutoDisposeNotifier`) went. Migration is optional, not mechanical.
- Retry fact-check **CONFIRMED**: `provider_container.dart:943` `int maxRetries = 10`, and `:948` `if (error is ProviderException || error is Error) return null;`. Retry is **bounded at 10** (~38s), not infinite. It *does* skip `Error` — but **`SqliteException` is an `Exception`, so a corrupt DB is still retried for ~38 seconds.** The `retry: (_, __) => null` recommendation survives, with the honest rationale.

### 1.9 Impeller: right conclusion, wrong population

performance says avoid gradients/SVG because *cheap old devices* fall back to GLES. The fact-check is right: both cited bugs (#179268, #177873) are defects of the **runtime Vulkan→GLES transition**, and #179268 explicitly does *not* reproduce on GLES-alone. Old/API-28 devices run GLES-alone — the configuration those issues report as *clean*. Keep "flat solid-color tiles" (it's free and aligns with zero-animation), but the reason is *denylisted Vulkan drivers on API 29+*, not device age.

### 1.10 QS tile storage — a three-way conflict on the crisis path

The pinned decision says "SharedPreferences." project-structure's fact-check says use `home_widget`. platform-channel-testing says own a versioned JSON file. **[VERIFIED]** shared_preferences 2.5.5: `SharedPreferencesAsync` defaults to **DataStore Preferences**, not `FlutterSharedPreferences.xml`; the legacy API "will be deprecated in the future" and prefixes every key with `flutter.`. So a Kotlin `getSharedPreferences("FlutterSharedPreferences", …).getString("phrase", …)` returns null **twice over** (wrong store, wrong key).

**Resolution:** platform-channel-testing is right — own the contract with an explicit versioned JSON file. It sidesteps both traps, survives a shared_preferences major, and is readable by a stranger. **But its test plan is wrong:** `SharedPreferences.setMockInitialValues` is in-memory only, so a Dart unit test asserting the mirror updated asserts a fake mutated a fake — green while the Kotlin read path is broken, manufacturing false confidence in precisely the silent failure it's meant to guard. The mirror invariant requires an `integration_test` writing from Dart and reading back through the real native path.

---

## 2. QUESTIONS A STAFF ENGINEER WOULD ASK THAT NOBODY ANSWERED

**2.1 Android auto-backup silently uploads the SQLite DB to Google Drive — and nothing in 12 dimensions mentions it.** This is the biggest hole in the corpus. `android:allowBackup` defaults to **true**. The app's boards — including the vocalization text, i.e. the most intimate content a user owns — get backed up to Google's servers with no code written. The privacy label says "no network, no server." The manifest says otherwise. iOS is the same story: the app documents directory goes to iCloud by default. An audience that "reads privacy labels adversarially" will find this.

And it's a genuine dilemma, not just a bug: constraint #2 says the data is irreplaceable and unmergeable — auto-backup is the only thing that would *save* a user's board when they lose their phone. So it is **privacy promise vs. data durability**, and it must be a deliberate, documented ADR (`allowBackup="false"` + `dataExtractionRules`, plus an explicit user-initiated export). Nobody named the tension. This deserves its own dimension.

**2.2 The crash log is a privacy artifact and nobody proposed redaction.** It's user-exportable, and the corpus's own `CrashLog.record(message, stack)` will happily capture vocalization text. A user exports a log to a developer and leaks "I need to leave, I'm not able to talk right now" plus whatever they typed. Needs a redaction rule and a test.

**2.3 "FIXED 3x4" is contradicted by the project's own schema.** The drift fact-check caught this and it was never propagated: `boards.grid_rows`/`grid_cols` exist, settings carries `grid_size`, and design docs require a 2x3 crisis layout. Yet drift's `customConstraints` example hard-codes `CHECK (row_index < 4 AND col_index < 3)`, the performance dimension asserts "Fixed 3x4 = 12 tiles. Never derived from data," and dart3-idioms hard-codes `kRows`/`kCols`. **Shipping the CHECK constraint makes the 2x3 layout a DB-level insert failure**, and it's baked into the primary key's table. Decide before v1 — this is exactly the migration you don't want at v2.

**2.4 First launch with no TTS engine at all.** The seed board renders, every tap is silent, and the app has no telemetry to tell you. What's the degraded UX? Nobody specified it, and it's the single most likely field state on a cheap Android device.

**2.5 Two TTS clients, one engine.** The QS tile speaks natively while the Flutter app may also be speaking. Nobody addressed arbitration, audio focus contention, or what "barge-in" means across process paths.

**2.6 Locale.** `voice_filter`'s example takes `localePrefix`. From where? Device locale? A setting? If a user's device is `fr-FR` and their phrases are English, filtering by device locale removes every working voice. Unanswered, and it's a total-silence path.

**2.7 Compose-field durability.** If the app is killed mid-typing, is the typed phrase lost? For someone in shutdown, retyping is extremely expensive. Nobody decided.

**2.8 OBF import/export is named as a decision and never tested.** It's the one place untrusted input enters the app, and it has no dimension.

---

## 3. UNVERIFIED APIs THAT WOULD FAIL ON COPY-PASTE

Ranked by blast radius.

1. **`overrideValue` does not exist. [VERIFIED]** testing-strategy's `pumpApp` helper — used by *every* widget test in that dimension — is `speechServiceProvider.overrideValue(...)`. The API is `overrideWithValue` (`provider.dart:114`). **This does not compile.** It's in the one helper the dimension tells you to write first.

2. **`simulatedAccessibilityTraversal(start:, end:)` is deprecated. [VERIFIED]** `controller.dart:167-180`: both `start` and `end` are `@Deprecated('Use startNode instead… deprecated after v3.15.0-15.2.pre')`. The a11y dimension's *self-declared highest-value test* uses the deprecated form. Correct form: `startNode:`/`endNode:` with `find.semantics.byLabel(...)` (**[VERIFIED]** `finders.dart:681`). Note the project sets `deprecated_member_use: warning`, so this ships *silently*.

3. **`close_sinks: error` in the shipped repo file is a no-op. [VERIFIED]** §1.2.

4. **`.fvmrc` is JSON, not YAML. [VERIFIED]** ci-release's finding text says *"`flutter: 3.44.0` works"*. The real parser (`setup.sh:107-113`):
   ```sh
   if [[ "$VERSION_FILE" == *".fvmrc" ]]; then VERSION="$(jq -r '.flutter' "$VERSION_FILE")"
   else VERSION="$(yq eval '.environment.flutter' "$VERSION_FILE")"; fi
   ```
   `flutter: 3.44.0` is the **pubspec.yaml `environment:`** form. In `.fvmrc` it must be `{"flutter": "3.44.0"}`. (The code example happens to be correct JSON; the prose is wrong — and prose is what gets followed.) Also **"exact versions only" is false**: `setup.sh:115` special-cases `stable`/`beta`/`master`/`main`. And **[VERIFIED]** flutter-action latest is `v2.23.0` (2026-03-25) — still v2.

5. **`containsSemantics` is deprecated. [VERIFIED]** `matchers.dart:858-862`, after v3.40.0-1.0.pre. `isSemantics` exists (`:1066`). This machine is 3.41.2 → live. Corpus mostly gets this right; ensure no example regresses.

6. **`MediaQueryData(...)` constructed from scratch** (a11y-testing's `pumpGrid`, testing-strategy's `pumpApp`) zeroes view-derived padding. widget-golden's `Builder` + `MediaQuery.of(context).copyWith(...)` is the correct form. Two dimensions ship the broken one.

7. **`physicalSize` takes physical pixels.** a11y-testing's `surface: Size(400, 800)` passed to `setSurfaceSize` is fine, but any `view.physicalSize = Size(320,568)` at default DPR 3.0 yields a **106×189** surface. Flagged in one fact-check, not propagated to the examples.

8. **`tester.container()`** — plausible: **[VERIFIED]** `RiverpodWidgetTesterX on WidgetTester` exists in flutter_riverpod 3.3.2. Not individually confirmed. **[OPEN]**

9. **`HomeWidgetPlugin.getData(context)`** (project-structure fact-check) — **[OPEN]**, and moot if §1.10 resolves to owned JSON.

10. **`dart run drift_dev make-migrations`** — **[OPEN]** on 2.34.x. Note the local cache is drift 2.31.0 while pub latest is **2.34.2 [VERIFIED]**; `schemaAt`/`startAt`/`migrateAndValidate`/`newConnection` all **[VERIFIED]** present in 2.31.0's `api/migrations_common.dart`, with `startAt` implemented as `schemaAt(v).then((s) => s.newConnection())` and the doc line *"If you want to insert data in a migration test, use [schemaAt]"* — the drift dimension's core claim is solid.

11. **`MinimumTextContrastGuidelineAAA` [VERIFIED]** `accessibility.dart:526`, `kAAAMinimumRatioNormalText = 7.0`. Good.

12. **Double-reported diagnostics [VERIFIED]** — every issue printed exactly twice under the plugin. Any CI issue-count threshold will be wrong.

---

## 4. ENTIRELY UNEXPLORED

- **Android auto-backup / iCloud backup vs. the privacy promise** (§2.1) — the largest gap; deserves a dimension.
- **Crash-log redaction** (§2.2).
- **OBF import/export**, the only untrusted-input surface.
- **Locale/voice-language selection** (§2.6) — a total-silence path.
- **Play Data Safety declaration** — named once in ci-release, never derived.
- **Variable grid dimensions** (§2.3) — contradicted across four dimensions.
- **`@useResult` / `unused_result`** — the official-architecture fact-check identifies this as where the real anti-silence value lives; it is **absent from the shipped `analysis_options.yaml`**. If you adopt sealed outcomes (§1.1), `unused_result: error` is the line that stops `await speak(text);` from silently discarding the outcome. `discarded_futures` does **not** cover it.
- **Cross-process audio arbitration** (§2.5).
- **Doze/battery-optimization effects on the QS tile**.
- **The `show_text` and `edit` surfaces** — widget-golden notes overflow only reports if a widget *paints*, so these need their own pumped tests. No dimension budgets them.

---

## Bottom line

The corpus is strong on reasoning and weak on **artifact coherence**. Three things must happen before any of it becomes a doc:

1. **Collapse the five `SpeechService` contracts into one** (sealed `SpeakOutcome`), and rewrite every fake to it. Right now a reader copy-pastes two dimensions and gets code that doesn't compile against itself.
2. **Fix the four verified copy-paste failures**: `overrideValue`→`overrideWithValue`, `start:`/`end:`→`startNode:`/`endNode:`, `close_sinks` needs `linter: rules:`, `.fvmrc` is JSON.
3. **Answer the auto-backup question.** Everything else in the corpus is engineering; that one is the privacy promise the whole product rests on, and twelve dimensions missed it.

The single highest-value correction is §1.2 + §1.1's fact-check read together: this project's two named safety nets — the analyzer and the tests — each currently contain a *verified* placebo (a `close_sinks` rule that never fires, and a "silence is impossible" test built on a throwing fake for a plugin that returns status codes). In a no-telemetry project, a safety net that reports green while checking nothing is worse than no net at all.
...
### CRITIQUE 3
Repo is empty (`RESEARCH.md`, `idea.md`, `analysis_options.yaml`, no `lib/`). SDK on this machine is **Flutter 3.41.2 / Dart 3.11.0**, not the 3.44/3.12 the brief assumes — every version claim below is pinned to what actually resolves. APIs verified live against drift's migration docs and api.flutter.dev (`SchemaVerifier.schemaAt`/`newConnection`, `--data-classes --companions`, `SemanticsController.simulatedAccessibilityTraversal`, `find.semantics.byLabel`, `isSemantics`).

---

# Test strategy: Offline AAC

## 0. The reframe that determines everything else

The instinct "no crash reporting, so write more tests" is **half wrong**, and acting on the wrong half will eat the two-week budget.

Tests and telemetry cover **disjoint** risk sets:

- **Tests cover risks you thought of.** More tests of things you already thought of add nothing to the channel Crashlytics used to provide.
- **Telemetry covered the risks you didn't think of** — device diversity, engine variants, the 4% of users with no TTS engine.

You cannot refill a deleted discovery channel with more of the same tests. Only three things substitute:

1. **A-priori enumeration of the hostile environment.** Normally you test the happy path and let telemetry find the environment. Here: **the environment IS the test suite.** §1 is that enumeration.
2. **Techniques that author cases you didn't write** — one seeded random loop over the edit-op space (§3.6). Not `glados` (v1.1.7, ~2 years stale).
3. **Making failure visible in-app**, then testing the visibility (§3.4). If you can't observe remotely, the user must be able to observe locally and hand you the log.

And the corollary that saves the budget: **integration tests do NOT substitute for telemetry.** They run on your one emulator — a single configuration — and sample zero device diversity. Worse, the Android emulator ships **no TTS engine**, so CI can never verify speech at all. Budget 3 integration tests, not a suite.

One structural point that outranks every test in this document:

> **A pre-migration file backup is a stronger safety net for user data than any quantity of migration tests, and it is ~15 lines.** Migration tests protect against bugs you enumerated. The backup protects against the migration bug you did not enumerate — which, with no telemetry, is the entire invisible category. Build it first. Then test it.

---

## 1. Silent failure taxonomy

`D`=Dart unit · `W`=widget · `I`=integration (device) · `M`=manual only · `X`=structurally untestable

### 1.1 Speech never happens

| # | Failure | Where | Mitigation |
|---|---|---|---|
| 1 | No TTS engine installed | **D** | Fake `voices()` throws → assert visible error, never silence |
| 2 | Engine present, zero voices | **D** | `offlineSafeVoices([]) == []` → UI must show "no voice" state |
| 3 | Only `network_required` voices, app is offline by design | **D** | `voice_filter`. **Android sends the STRING `"1"`/`"0"`.** `raw['network_required'] == true` is always false (String vs bool) and `"0"` is non-empty so it survives truthiness checks — a naive check **silently inverts the safety property** in the direction that hurts |
| 4 | `setVoice` returns 0 | **D** + 1 contract test | Confirmed verbatim in `FlutterTtsPlugin.kt:514-526`: `Log.d(tag, "Voice name not found: $voice"); result.success(0)`. It is `result.success`, not `result.error` — **it never throws.** Check `== 1` |
| 5 | Stored voice uninstalled since last launch (Android GCs voice data) | **D** | **Most probable real-world silent failure.** Re-resolve the stored voice id against `voices()` at startup; fall back audibly |
| 6 | Voice has `notInstalled` feature | **D** (filter) / **M** (behavior) | `setVoice` returns **1 (success)** and synthesis reports `ERROR_NOT_INSTALLED_YET` *or substitutes a different voice*. **The return-value check does not catch this.** Filter on `features.contains('notInstalled')`. Features are **TAB-separated** |
| 7 | `speak()` returns 0 | **D** | Check `== 1` |
| 8 | Engine reports success, emits no audio | **M** | No hook exists to capture PCM from AVSpeechSynthesizer/Android TTS. Checklist §5, item 1 |
| 9 | `.ambient` audio session → silent switch mutes the app | **M** | **flutter_tts's own README example uses `.ambient`.** A Dart test can only assert *your code passed `.playback` to the wrapper* — a value-level assertion, not the real `AVAudioSession` category. Checklist §1 |
| 10 | Audio focus denied (call in progress) | **M** | Checklist §1 |
| 11 | Bluetooth yanked mid-utterance | **M** | Checklist §1 |
| 12 | TTS service bind latency (seconds) reads as silence | **M** | Warm the engine from `addPostFrameCallback`, never awaited on the path to first frame. `flutter_tts` PR #594 documents binder IPC + voice deserialization running **synchronously on the main thread** in `OnInitListener` |
| 13 | Android 11+ package visibility hides the TTS engine | **D** ← *cheap and non-obvious* | Missing `<queries><intent><action android:name="android.intent.action.TTS_SERVICE"/></intent></queries>` → empty voice list, total silence, every Android 11+ device. **A Dart test can read the manifest file.** §3.7 |

### 1.2 The wrong thing is spoken

| # | Failure | Where | Mitigation |
|---|---|---|---|
| 14 | Tile speaks its **label** instead of its **vocalization** | **W** | The core Open Board Format semantic. Nothing in the type system distinguishes two `String`s |
| 15 | Stale closure: rapid re-tap speaks the *previous* tile's phrase | **W** | Never capture a `ref.watch` value in `build()` into an `onTap` closure. Pass `(row, col)` — position is the PK and can never go stale |
| 16 | QS tile speaks a phrase deleted months ago (mirror drift) | **I only** | See §1.5 |
| 17 | Tile reflow → user's muscle memory hits the wrong tile | **D** + **W** | Schema makes it structurally impossible; tests pin it anyway |

### 1.3 The board is gone

| # | Failure | Where | Mitigation |
|---|---|---|---|
| 18 | Migration produces correct schema, drops all rows | **D** | `migrateAndValidate` is **blind to rows** — §4 |
| 19 | User skipped 6 months of updates: v1→v3 path never tested | **D** | Nested loop, ~10 lines, highest-value DB test |
| 20 | Schema changed without bumping `schemaVersion` → no migration runs | **CI** | `drift_dev schema dump` + `git diff --exit-code` |
| 21 | **SQLite FKs are OFF by default and are per-connection** | **D** | `grid_slots.button_id` is a nullable FK whose entire purpose is `onDelete: KeyAction.setNull`. With FKs off, SQLite **silently ignores the action** → dangling `button_id` → blank-or-crashing tile |
| 22 | Absolute image path dies on iOS reinstall/restore (container UUID changes) | **D** | Store paths **relative** to app documents dir |
| 23 | 12MP photo × 12 tiles = OOM kill on a 2GB phone | **D** | Downscale **at import** (max 512px), not at render. An OOM is permanently invisible without telemetry |

### 1.4 The failure exists but is not surfaced

| # | Failure | Where | Mitigation |
|---|---|---|---|
| 24 | Error caught and swallowed | **W** | §3.4 — the parameterized test |
| 25 | Crash log throws inside the error handler → recursion | **D** | §3.5 |
| 26 | Crash log buffered, not flushed → the startup crash you most need is lost | **D** | Synchronous, `flush: true` |
| 27 | **`onTap: () => tts.speak('x')` drops the Future — caught by NO lint** | **structural** | Empirically verified on this machine's toolchain: the arrow closure "returns" the Future so `discarded_futures` considers it handled, but the target type is `VoidCallback`, so the Future *and its error* are dropped. **This is the single most idiomatic way to wire a tile and it is exactly the silence bug.** Fix by construction: a **void-returning** `SpeechController.speakNow(String)` that internally does `unawaited(_speak(p).catchError(...))`. A callback then never holds a Future |

### 1.5 The Quick Settings tile — the structural hole

The QS TileService runs with **no Flutter engine** by design. **No Flutter test of any level can reach it.** State this plainly rather than papering over it: this is the app's crisis path and it has zero Dart-side coverage.

Worse, the obvious contract test **does not work**:

> `SharedPreferences.setMockInitialValues` is **in-memory only** — it "will not persist values to the usual preference store." A Dart test asserting "edit a tile → the pref changed" asserts *a fake mutated a fake*. It passes green while the Kotlin read path is broken — manufacturing false confidence in precisely the failure it claims to guard.

Two more landmines on this path:
- Modern `SharedPreferencesAsync` is backed by **Jetpack DataStore**, not `FlutterSharedPreferences.xml`. Kotlin's `getSharedPreferences(...)` reads **nothing**.
- The legacy API prefixes every key with `flutter.` — native `getString("phrase")` returns null; the key is `flutter.phrase`.

**Mitigation (three parts, all required):**
1. **Own the contract.** Don't couple the tile to a plugin's private storage format. Write an explicit versioned JSON file from Dart; read that file from Kotlin. One Dart file and one Kotlin object are the sole owners.
2. **`integration_test` round-trip** — write from Dart, read back through the *real* native path, assert byte equality. This is the only thing that catches renames, prefix errors, backend misconfiguration, and encoding drift.
3. **Extract the logic out of `TileService`.** `onClick()` should be ~5 lines delegating to a plain Kotlin class (read → validate non-empty → speak) that plain JUnit tests with a fake TTS. No Robolectric — `TileService` is lifecycled by SystemUI and has no first-class shadow.

---

## 2. What is NOT worth testing
...
| A golden regression suite | Every failure it would catch (text not fitting, tiles reflowing) is caught more cheaply and with a *readable message* by the overflow matrix + `getRect` invariants. And: goldens are platform-dependent, so **a stranger cloning on Linux against macOS goldens sees a wall of red and concludes the repo is broken** — goldens actively sabotage the open-source exit plan |
| Patrol / Appium / any E2E | No accounts, no network, one screen, no runtime permission prompts |
| A coverage **percentage** gate | VGV's own rationale is confidence-under-change; the target is defensible for a consultancy, not for a 2-week solo budget. Gate the **four danger files** at 100% instead. Note `flutter test --coverage` **omits files no test imports**, so the number lies **upward** (flutter#27997) — worse than no number |
| Firebase Test Lab / device farms | Farm devices are the same TTS-poor images. Cannot assert audio. One real phone strictly dominates |

---

## 3. The minimum suite

**~135 tests, < 30 seconds.** The runtime number is not vanity — it is what determines whether a solo dev keeps running them.

| File | Tests | Covers |
|---|---:|---|
| `test/data/migration_test.dart` | ~6 (grows O(V²)) | §4 |
| `test/data/backup_test.dart` | 4 | Backup happens **before** `onUpgrade`; last-2 retention; restore round-trip; restore when DB is corrupt |
| `test/data/database_test.dart` | 12 | §3.1 |
| `test/data/grid_invariant_test.dart` | 1 (×200 seeds) | §3.6 |
| `test/data/media_store_test.dart` | 5 | Relative paths; 4000×3000 import → ≤512px stored; missing file → visible placeholder |
| `test/speech/voice_filter_test.dart` | 14 | §3.2 |
| `test/speech/speech_service_test.dart` | 10 | Every `!= 1` return path → typed failure |
| `test/speech/silence_is_impossible_test.dart` | ~10 | §3.4 — **the highest-value test** |
| `test/native/tts_channel_contract_test.dart` | 4 | §3.3 |
| `test/diagnostics/crash_log_test.dart` | 8 | §3.5 |
| `test/ui/board_test.dart` | 15 | Label≠vocalization; barge-in ordering; empty slot not tappable; stale-closure |
| `test/ui/a11y_test.dart` | ~14 | §5 |
| `test/ui/overflow_matrix_test.dart` | 30 | 3 devices × 5 scales × 2 bold |
| `test/policy/*.dart` | 4 | §3.7 |
| `integration_test/` | 3 | Boot+speak; DB survives restart; **QS-tile pref round-trip** |

**The four 100%-coverage files** (a bug here is unrecoverable, not annoying):
`lib/data/database/` (migrations) · `lib/speech/voice_filter.dart` · `lib/data/repositories/board_repository.dart` · `lib/diagnostics/crash_log.dart`

### 3.0 Test doubles: fakes, not mocks

Use a **hand-written fake** implementing `SpeechService`. Not because mocks "silently absorb" — that's overstated; mocktail throws a `TypeError` on an un-stubbed `Future`-returning method. The real reasons, and Flutter's own architecture docs demonstrate exactly this pattern:

1. `implements` **without** a `noSuchMethod` superclass = adding a method to the interface **breaks the build**.
2. The risk isn't "was `speak()` called" — it's "what happens when the voice vanished / `setVoice` returned 0." Those are **state**; a fake models state naturally where a mock needs `when`+side-effect gymnastics.
3. The fake **is** the executable documentation of the contract, for the stranger who inherits this.

**Never mock the database.** 100% of DB risk lives in SQL/schema/migrations; a Map-backed fake happily accepts a row that `PRIMARY KEY (board_id, row, col)` rejects and never executes a migration step. `NativeDatabase.memory()` is real sqlite3 — it isn't even a fake.

> **Document the host-sqlite3 requirement in README.** `flutter test` runs in a plain Dart VM; `sqlite3_flutter_libs` does **nothing**. Ubuntu needs `apt-get install libsqlite3-dev` *before* `flutter test`. A stranger who clones on Linux and sees DB tests fail concludes the repo is broken and walks away — that's the exit plan dying to a missing README line.

### 3.1 Database invariants — the architecture, executable
...
setUp(() {
  // The REAL AppDatabase, so beforeOpen — and therefore FK enforcement — applies.
  // A bare connection would skip it and let FK-violating code pass green.
  db = AppDatabase(DatabaseConnection(
    NativeDatabase.memory(),
    // REQUIRED. drift keeps an unsubscribed query stream open for one event-loop
    // iteration, which never arrives under a widget test's FakeAsync clock.
    closeStreamsSynchronously: true,
  ));
});
tearDown(() => db.close());
...
test('deleting a button empties its slot without moving any other tile', () async {
  final id = await db.into(db.buttons).insert(ButtonsCompanion.insert(
      label: 'Overwhelmed',
      vocalization: const Value("I need to leave, I'm not able to talk right now")));
  await (db.update(db.gridSlots)
        ..where((s) => s.rowIndex.equals(2) & s.colIndex.equals(1)))
      .write(GridSlotsCompanion(buttonId: Value(id)));

  await (db.delete(db.buttons)..where((b) => b.id.equals(id))).go();

  final slots = await db.select(db.gridSlots).get();
  expect(slots, hasLength(12), reason: 'The slot must survive its button.');
  expect(slots.firstWhere((s) => s.rowIndex == 2 && s.colIndex == 1).buttonId, isNull,
      reason: 'This is ON DELETE SET NULL working. If FKs were off it would '
              'still be $id — a dangling id pointing at nothing.');
});

test('a slot cannot be occupied twice — reflow is structurally impossible', () async {
  await expectLater(
    db.into(db.gridSlots).insert(
        GridSlotsCompanion.insert(boardId: 1, rowIndex: 0, colIndex: 0)),
    throwsA(isA<SqliteException>()),
...
The FK test and the delete test are a **pair**: the delete test is the behavioural consequence, the PRAGMA test tells you *why* it broke.

`PRAGMA foreign_keys` is a **silent no-op inside a transaction** — it does not error, it does nothing. So in `onUpgrade`: `PRAGMA foreign_keys = OFF` *outside* the transaction, migration inside, `PRAGMA foreign_key_check` after, and re-enable **unconditionally in `beforeOpen`** (not inline, not inside `if (details.wasCreated)` — the pragma is per-connection and must run on every open).

### 3.2 voice_filter — the four wire-format traps

All four confirmed in plugin source. Any one of them silently inverts the safety property.

```dart
// lib/speech/voice_filter.dart
const kFeatureNetworkTts   = 'networkTts';
const kFeatureNotInstalled = 'notInstalled';
...
class Voice {
  ...
  static Voice? tryParse(Object? raw) {
    if (raw is! Map) return null;
    final name = raw['name'], locale = raw['locale'];
    if (name is! String || name.isEmpty) return null;
    if (locale is! String || locale.isEmpty) return null;

    // TRAP 1: Android sends the STRING "1"/"0" (FlutterTtsPlugin.kt:623).
    //         "0" is non-empty => truthy to a careless check.
    //         `raw['network_required'] == true` is ALWAYS false (String vs bool).
...
// TRAP 4: getVoices can return null — the plugin catches NPE and calls success(null).
List<Voice> offlineSafeVoices(Object? raw) {
  if (raw is! List) return const [];
  return raw.map(Voice.tryParse).whereType<Voice>().where((v) => v.isOfflineSafe).toList();
}
```
...
Map<Object?, Object?> androidVoice(String name, String locale,
        {bool network = false, List<String> features = const []}) =>
    <Object?, Object?>{
      'name': name, 'locale': locale, 'quality': 'normal', 'latency': 'normal',
      'network_required': network ? '1' : '0',   // STRING
      'features': features.join('\t'),           // TAB
    };

Map<Object?, Object?> iosVoice(String name, String locale) => <Object?, Object?>{
      'name': name, 'locale': locale, 'quality': 'default', 'gender': 'female',
...
### 3.3 The ONE channel-level test — an upgrade canary

**Do not mock the flutter_tts channel across the suite.** Fake `SpeechService` instead. Channel mocks couple 50 tests to the plugin's private method-name strings and prove nothing about audio. Confine it to one file whose job is to **pin the plugin's wire behavior** so an upgrade that changes it fails loudly. With no telemetry, this is your only upgrade canary.

```dart
// test/native/tts_channel_contract_test.dart
void main() {
  // REQUIRED before touching TestDefaultBinaryMessengerBinding.instance in a
  // plain test(). testWidgets does this implicitly; test() does not.
  // Omit it and the file fails to LOAD: "Binding has not yet been initialized."
  TestWidgetsFlutterBinding.ensureInitialized();
  ...
  setUp(() => messenger =
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger);
  tearDown(() => messenger.setMockMethodCallHandler(channel, null)); // or it leaks

  test('setVoice returning 0 does NOT throw — it silently succeeds', () async {
    messenger.setMockMethodCallHandler(channel,
        (call) async => call.method == 'setVoice' ? 0 : null);
    final result = await channel.invokeMethod<dynamic>(
        'setVoice', {'name': 'ghost', 'locale': 'en-US'});
    expect(result, 0); // The entire bug in one line: no exception, just a 0.
  });

  test('getVoices is untyped — the naive cast is a runtime crash', () async {
    messenger.setMockMethodCallHandler(channel,
        (call) async => <Object?>[<Object?, Object?>{'name': 'a', 'locale': 'en-US'}]);
    final raw = await channel.invokeMethod<dynamic>('getVoices');
    expect(() => (raw as List).cast<Map<String, String>>().first,
...
### 3.4 THE test — silence is impossible
...
enum SpeechFailure {
  ...
  setVoiceReturnedZero,    // flutter_tts logs Log.d and returns 0
  storedVoiceUninstalled,  // Android GC'd the voice since last launch
  voiceNotInstalledFeature,// setVoice returns 1 and it still won't speak
  reportedSuccessButSilent,
  audioFocusDenied,
}
...
void main() {
  for (final failure in SpeechFailure.values) {
    testWidgets('tile tap under $failure: speech OR visible error, never neither',
        (tester) async {
      final speech = FakeSpeechService()..failWith = failure;
      tester.useDevice(Device.small);          // pin a REAL phone, see §5
      await tester.pumpApp(speech: speech);

      await tester.tap(find.bySemanticsLabel('Overwhelmed'));
      await tester.pump();                     // one frame. zero animation.

      final spoke = speech.spoken.isNotEmpty;
      final showedText = find.text("I need to leave, I'm not able to talk right now")
          .evaluate().isNotEmpty;
      final showedError = find.byKey(const Key('speech-error-banner'))
          .evaluate().isNotEmpty;

      expect(spoke || showedText || showedError, isTrue,
          reason: 'SILENT FAILURE under $failure: a user tapped a tile mid-shutdown '
                  'and got neither speech nor an on-screen fallback. '
                  'Nobody will ever report this to you.');
    });
  }
}
```

**Why this is the highest-value test in the app:** it is *structurally unsatisfiable by a code path that fails silently*, and adding a value to `SpeechFailure` forces the UI to handle it or the suite goes red. It converts the project's stated worst bug class from a principle into a mechanism.

Model the outcome as a **sealed** hierarchy whose failure variants **carry the fallback payload** (`final String spokenText`), so showing the text is a *total function* of the outcome:
...
sealed class SpeakFailure extends SpeakOutcome {
  const SpeakFailure(this.spokenText);
  final String spokenText;   // the on-screen fallback, always present
  String get logLine;
}
final class VoiceUnavailable extends SpeakFailure { /* ... */ }
final class EngineRejected   extends SpeakFailure { /* ... */ }

// The call site. No `default:`, no `case _:`.
switch (outcome) {
  case SpokeAloud(): return;
  case SpeakFailure(:final spokenText, :final logLine):
    _log.record('speak failed: $logLine', StackTrace.current);
    _showText(spokenText);
}
...
**Be precise about what this buys.** A non-exhaustive switch on a sealed type **is a compile error** (`non_exhaustive_switch_statement` is `type: compileTimeError` in the analyzer's `messages.yaml` — widespread claims that it's a mere warning are wrong; don't let a reviewer talk you out of this). But exhaustiveness forces a *branch*, not an *action*, and does **not** force the caller to switch at all — `await speak(text);` discarding the outcome compiles clean. To close that second hole: `@useResult` from `package:meta`, which yields the `unused_result` **analyzer** diagnostic. Promote it to `error` in `analysis_options.yaml`. The honest guarantee is *"compile-error on new variants + analyzer-error on discarded outcomes"* — not "silence is impossible."

And nothing in the type system detects `setVoice` returning 0. **You must hand-write that check.** `Result`/sealed types only guarantee the failure *propagates* once detected. The detection gap is the actual root cause.

### 3.5 The crash log — test it harder than the tiles

It is the only remaining line of sight into the field. That makes it load-bearing infrastructure, not a nice-to-have. Nobody tests their crash logger.

```dart
test('an error inside the logger does not re-enter the logger', () { /* recursion guard */ });
test('a crash on the first frame still leaves a readable log', () { /* flushed, not buffered */ });
test('the log is bounded and never fills the disk', () { /* size cap */ });
test('ProviderException is unwrapped to the real cause', () { /* else every entry reads "ProviderException" */ });
test('export works when the app cannot fully start', () { /* ... */ });
```

Wiring, verified against Flutter's own error docs:
...
void main() async {
  ...
  PlatformDispatcher.instance.onError = (error, stack) {
    try {
      log.record(error.toString(), stack);
      if (kDebugMode) debugPrint('$error\n$stack');  // your visibility, not the embedder's
    } catch (_) { /* never let the handler throw */ }
    return true;   // NEVER take the false branch
  };
...
Two corrections to widely-circulating advice worth encoding here:

- **No `runZonedGuarded`.** Flutter's zone-errors breaking-change doc says the fix for the zone-mismatch warning is to *remove zones from the application*. The "use all three handlers" advice is **crash-SDK advice** — Sentry needs a zone because it wraps init. You have no SDK, so the zone is pure footgun. (This contradicts one line elsewhere in the corpus; the official doc wins.)
- **Return `true` always, not `kReleaseMode`.** `api.flutter.dev` explicitly warns the `false` path routes through the embedder's `unhandled_exception_callback` and "the VM or the process may exit or become unresponsive." `kReleaseMode` buys debug visibility at the cost of the one behavior a crisis UI cannot tolerate — and buys it unnecessarily, since `debugPrint` gives the same visibility for free.

The bare catch inside `CrashLog.record` **deliberately violates** Effective Dart's "DON'T discard errors" rule. The comment explaining why is load-bearing — without it, someone "fixes" it into infinite recursion.

### 3.6 The substitute for unknown-unknowns

```dart
// test/data/grid_invariant_test.dart
test('no sequence of edit ops ever reflows a tile', () async {
  for (var seed = 0; seed < 200; seed++) {
    ...
    try {
      final expected = <(int, int), int?>{};
      for (var i = 0; i < 30; i++) {
        final r = rng.nextInt(4), c = rng.nextInt(3);
        if (rng.nextBool()) {
        ...
        for (final e in expected.entries) {
          expect(slots[e.key], e.value, reason: 'TILE REFLOWED at ${e.key}. seed=$seed ops=$ops');
        }
...
This is the only technique that authors cases *you didn't write* — the actual thing Crashlytics provided. You lose automatic shrinking; the printed op list **is** the minimal repro you'd read anyway.

### 3.7 Policy tests — enforcing what no lint can

There are **zero accessibility lints** in `flutter_lints` or `very_good_analysis`. The only free candidate (`accessibility_lint`) is abandoned — 0 likes, 183 downloads, 19 months stale, built on the deprecating `custom_lint`. The credible rules are in commercial DCM, which a stranger can't run.

So: source-grep tests. ~10 lines each, and they make the constraint unbreakable by a future contributor (or you at 2am).

```dart
// test/policy/no_text_clamping_test.dart
test('no code clamps or overrides text scaling', () {
  const forbidden = ['withClampedTextScaling', 'textScaleFactor'];
  final offenders = <String>[];
  for (final f in Directory('lib').listSync(recursive: true).whereType<File>()) {
    if (!f.path.endsWith('.dart')) continue;
    final src = f.readAsStringSync();
    for (final bad in forbidden) if (src.contains(bad)) offenders.add('${f.path}: $bad');
  }
  expect(offenders, isEmpty,
      reason: 'TextScaler must be honored at 200%+ and never clamped.\n'
              'Fix the layout, do not clamp the text.\n${offenders.join("\n")}');
...
`MediaQuery.withClampedTextScaling` is the **one-line "fix" a future contributor reaches for when an overflow test fails** — and it silently defeats the entire overflow matrix while contrast and tap-size still pass.

```dart
// test/policy/android_manifest_test.dart  <- cheap, non-obvious, prevents TOTAL silence
test('TTS_SERVICE is declared in <queries> — Android 11+ package visibility', () {
  ...
  expect(xml, contains('android.intent.action.TTS_SERVICE'),
      reason: 'Without this, Android 11+ HIDES the TTS engine. flutter_tts returns '
              'an empty voice list with only a Log.d. Every Android 11+ user gets '
              'a board that cannot speak, and you will never hear about it.');
});
```

```dart
// test/policy/no_pump_and_settle_test.dart
// Zero animation means every state change settles in ONE frame. pumpAndSettle can
// only add a 10-minute-timeout flake vector with truncated stack traces (#84966).
// LOCAL CONVENTION, not a sourced Flutter recommendation. And note pump() does NOT
// advance the fake clock — time-based async still needs pump(duration) or fakeAsync.
```

Plus a **"nothing animates"** test — the useful inversion:

```dart
testWidgets('no widget schedules a second frame', (tester) async {
  await tester.pumpApp();
  for (final label in kSeedBoardLabels) {
    await tester.tap(find.bySemanticsLabel(label));
    await tester.pump();
...
Honest scope: this is a **spot check**, not proof. It misses Timer-driven repaints and at-rest animations, and catches a stray `InkWell` only on tiles you tap. And the remedy is bigger than folklore suggests: `splashFactory: NoSplash.splashFactory` kills only the *splash* — `InkResponse.updateHighlight()` independently creates an `InkHighlight` with a 200ms pressed fade. You must also neutralize `overlayColor`/`highlightColor`, or just use `GestureDetector`.

---

## 4. Migration testing: what "the phrases survive" actually requires

### 4.1 The gap

**`migrateAndValidate` extracts `CREATE` statements from `sqlite_schema` and semantically compares them to a reference built by `Migrator.createAll`. It is a *shape* comparison. It does not look at rows.**

A migration that rebuilds `grid_slots` perfectly and copies **zero rows** passes it, green.

Six more things it does not cover:
1. **Non-adjacent paths.** The user who ignored updates for six months goes v1→v3 directly — the untested path.
2. **Data encoding.** The `store_date_time_values_as_text` flag is a v1-only decision; toggling it later is "not compatible with existing database schemas."
3. **FK enforcement state** — a connection property, not a schema property.
4. **The backup.**
5. **Referential integrity while FKs were off during the migration.**
6. **The `schemaAt` fixture does not run your `beforeOpen`** — it uses `Migrator.createAll` from the export, so FKs are off while you write fixture data. Fine, but name it: you can seed states the real app couldn't create.

### 4.2 The design

```bash
# build.yaml — do this at commit #1, before there is data to lose.
targets:
  $default:
    builders:
      drift_dev:
        options:
          databases: { app_database: lib/data/database/app_database.dart }
          test_dir:   test/drift/
          schema_dir: drift_schemas/
```

`dart run drift_dev make-migrations` — one command, supersedes the old `schema dump`/`steps`/`generate` triad. It emits schema JSONs, `database.steps.dart` (the `stepByStep` helper), **and** migration tests. Drift's own docs: *"Writing migrations manually is error-prone and can lead to data loss."*

**Commit `drift_schemas/`.** Not because the history is otherwise unrecoverable — it isn't; `schema dump` reads the Dart source, so you can check out the tag and regenerate. Commit it because it makes schema deltas a **reviewable diff** and removes a git-archaeology step from a future migration.

**CI gate** — catches the bug that loses boards without touching a single line of migration code:
...
Requires: `dart run drift_dev schema generate --data-classes --companions drift_schemas/ test/drift/generated/`
...
/// A hostile, realistic board. Not "test1"/"test2".
const kTorture = [
  ('Overwhelmed', "I need to leave, I'm not able to talk right now"),  // apostrophe
  ('Nej',         'Nej tack — jag kan inte prata just nu'),            // non-ASCII
  ('🚻',          'I need the toilet'),                                // emoji label
  ('Long',        '<400 chars of prose>'),                            // truncation bait
  ('Quote',       'She said "no" — and I agree; see: a\\b'),           // quote/escape bait
  ('Empty-ish',   ' '),                                               // whitespace-only
];

group('schema shape', () {
  // Every hop. Necessary but NOT sufficient.
  for (var from = 1; from < kLatestSchemaVersion; from++) {
    for (var to = from + 1; to <= kLatestSchemaVersion; to++) {
      test('migrates v$from -> v$to', () async {
        final conn = await verifier.startAt(from);
...
// THE test. A botched migration here is the loss of someone's voice, and with no
// telemetry this assertion is the only thing that will ever tell us it broke.
test('v1 -> v2 preserves every hand-curated phrase, byte for byte, in place', () async {
  final schema = await verifier.schemaAt(1);
  ...
  final boardId = await old.into(old.boards).insert(v1.BoardsCompanion.insert(name: 'Home'));
  for (var i = 0; i < kTorture.length; i++) {
    final (label, vocalization) = kTorture[i];
    final bid = await old.into(old.buttons).insert(
        v1.ButtonsCompanion.insert(label: label, vocalization: Value(vocalization)));
    await old.into(old.gridSlots).insert(v1.GridSlotsCompanion.insert(
        boardId: boardId, rowIndex: i ~/ 3, colIndex: i % 3, buttonId: Value(bid)));
  }
  // An EMPTY slot must survive as an empty slot — not vanish, not collapse.
  await old.into(old.gridSlots).insert(v1.GridSlotsCompanion.insert(
      boardId: boardId, rowIndex: 3, colIndex: 2));
  await old.close();

  // --- Run the REAL migration against that data --------------------------
  final db = AppDatabase(schema.newConnection());
  await verifier.migrateAndValidate(db, 2);           // shape
  final violations = await db.customSelect('PRAGMA foreign_key_check').get();
  expect(violations, isEmpty,
      reason: 'Migration left dangling FKs while enforcement was off. '
              'A dangling board_id is a lost board.');
  await db.close();

  // --- Read back with v2-era classes and assert CONTENT -------------------
  final now = v2.DatabaseAtV2(schema.newConnection());
  final buttons = await now.select(now.buttons).get();
  final slots   = await now.select(now.gridSlots).get();

  expect(slots, hasLength(kTorture.length + 1), reason: 'A slot disappeared.');
  expect(slots.where((s) => s.buttonId == null), hasLength(1),
      reason: 'The empty slot must survive as an empty slot.');

  for (var i = 0; i < kTorture.length; i++) {
    final (label, vocalization) = kTorture[i];
    final slot = slots.firstWhere((s) => s.rowIndex == i ~/ 3 && s.colIndex == i % 3);
    final btn  = buttons.firstWhere((b) => b.id == slot.buttonId);

    expect(btn.label, label);
    expect(btn.vocalization, vocalization,
        reason: 'The vocalization is the whole point. Losing it is losing speech. '
                'Truncation, re-encoding, and quote-mangling all land here.');
  }
  await now.close();
});
```

### 4.4 The thing that actually saves the boards

```dart
/// Copy the .sqlite file to board_backup_v{oldVersion}.sqlite immediately before
/// onUpgrade runs. Keep the last 2. Expose "Restore previous board" in settings.
///
/// ~15 lines. The single highest safety-per-line item in the project, and it is
/// not a test at all. Migration tests protect against bugs you enumerated; this
/// protects against the migration bug you did not — which with no telemetry is
/// the entire category you cannot see.
```

Then test **the backup path itself** (4 tests, §3): ordering (backup strictly before `onUpgrade`), retention, restore round-trip, restore-from-corrupt.

---

## 5. Accessibility: automated vs. manual

### 5.1 The uncomfortable truths, verified

**(a) `MinimumTapTargetGuideline` is close to a no-op on this app's geometry.**

Source (`packages/flutter_test/lib/src/accessibility.dart`): `if (_isAtBoundary(paintBounds, viewRect)) { return result; }` — it **silently skips any tappable node whose paint bounds touch the view edge** (gap threshold 0.001, all four sides). On a full-bleed 3×4 grid, **the 10 perimeter tiles are skipped and only the 2 interior tiles are measured.** The test goes green while checking almost nothing.

→ **Assert tile geometry directly with `tester.getSize()`.** No skip logic, no false pass, and a failure message that names the tile and prints the actual dp — which is the difference between a stranger fixing it in 30 seconds and giving up.

**(b) `textContrastGuideline` has an open, unfixed false negative.** flutter/flutter#103235: white text (`0xffffff`) on `0xfafafa` **PASSES**. It screenshots the layer and picks fg/bg from a colour histogram, mis-attributing background in low-variance regions.

→ **Compute contrast in a pure-Dart unit test** over your theme's colour pairs. ~10 lines, deterministic, cannot false-pass.

**(c) The default test surface is 800×600 logical — wider than any phone.** An unpinned overflow test measures a screen no user owns. And `physicalSize` takes **physical** pixels: `Size(360*3, 800*3)` with `devicePixelRatio = 3.0`.

**(d) Overflow reports only ONCE per RenderObject** (`_overflowReportNeeded`, reset only on `reassemble`). Looping scales inside one `testWidgets` silently under-reports scales 2 and 3.

→ **Generate one `testWidgets` per (device × scale × bold) tuple.**

**(e) `containsSemantics` is deprecated** after v3.40.0-1.0.pre in favour of `isSemantics` — verified present on api.flutter.dev. Virtually every tutorial predates this.

### 5.2 What the automated suite genuinely covers

```dart
// test/ui/a11y_test.dart

// (1) GEOMETRY — the real gate. Does NOT use meetsGuideline.
testWidgets('every one of the 12 tiles is at least 76x76 dp', (tester) async {
  tester.useDevice(Device.seLike);          // 320x568 — worst case we support
  await pumpGrid(tester, textScaler: const TextScaler.linear(2.0));
  for (final t in kTestTiles) {
    final size = tester.getSize(find.byKey(ValueKey('tile_${t.row}_${t.col}')));
    expect(size.width,  greaterThanOrEqualTo(76.0),
        reason: '"${t.label}" is ${size.width}dp wide; min tap target is 76dp');
    expect(size.height, greaterThanOrEqualTo(76.0));
  }
});

// (2) LABEL CORRECTNESS — the check NO guideline makes.
// labeledTapTargetGuideline only checks a label is non-empty. A tile labeled
// "button1" passes it. So does a tile that leaks the whole vocalization.
testWidgets('every tile announces its DISPLAY label, never its vocalization',
    (tester) async {
  await pumpGrid(tester);
  for (final t in kTestTiles) {
    final node = tester.getSemantics(find.byKey(ValueKey('tile_${t.row}_${t.col}')));
    expect(node, isSemantics(                 // NOT containsSemantics — deprecated
      label: t.label, isButton: true, hasEnabledState: true,
      isEnabled: true, isFocusable: true, hasTapAction: true, isHidden: false,
    ));
    expect(node.label, isNot(contains(t.vocalization)),
        reason: 'tile "${t.label}" leaks its vocalization into the semantic label; '
                'a screen reader user hears the whole sentence on every scan step');
  }
});

// (3) TRAVERSAL ORDER — the best available proxy for switch scanning.
testWidgets('screen reader visits the 12 tiles row-major', (tester) async {
  await pumpGrid(tester);
  final ordered = tester.semantics.simulatedAccessibilityTraversal(
    startNode: find.semantics.byLabel('Overwhelmed'),   // FinderBase<SemanticsNode>
    endNode:   find.semantics.byLabel('Write'),
  );
  expect(ordered.map((n) => n.label).toList(), kTestTiles.map((t) => t.label).toList(),
      reason: 'Tile traversal order changed. This breaks muscle memory for screen '
              'reader AND switch users, who scan in this same order.');
});

// (4) ANTI-CLAMP — no guideline catches a clamped TextScaler.
// Contrast and tap-size both still pass while text stops growing.
testWidgets('text scale is honored, never clamped', (tester) async {
  await pumpGrid(tester, textScaler: TextScaler.noScaling);
  final base = tester.getSize(find.text('Overwhelmed')).height;
  await pumpGrid(tester, textScaler: const TextScaler.linear(2.0));
  expect(tester.getSize(find.text('Overwhelmed')).height, greaterThan(base * 1.8),
      reason: 'Text did not grow at 2.0x — someone clamped TextScaler to keep the '
              'grid tidy. 200%+ must be honored.');
});

// (5) LAYOUT INVARIANT — the golden replacement.
testWidgets('grid keeps 12 slots in fixed positions at 200%', (tester) async {
  tester.useDevice(Device.small);
  await pumpGrid(tester, textScaler: const TextScaler.linear(2.0));
  Rect slot(int r, int c) => tester.getRect(find.byKey(ValueKey('slot_${r}_$c')));
  for (var r = 0; r < 4; r++) {
    for (var c = 0; c < 3; c++) {
      // Mirrors PRIMARY KEY (board_id,row,col) at the UI layer.
      expect(slot(r, c).top,  moreOrLessEquals(slot(r, 0).top,  epsilon: 0.5));
      expect(slot(r, c).left, moreOrLessEquals(slot(0, c).left, epsilon: 0.5));
    }
  }
...
// (6) ADVISORY only. Cheap tripwire. Never the gate.
testWidgets('meets built-in guidelines (advisory)', (tester) async {
  final handle = tester.ensureSemantics();
  addTearDown(handle.dispose);   // addTearDown, not a trailing call — survives a throw
  await pumpGrid(tester);
  await expectLater(tester, meetsGuideline(labeledTapTargetGuideline));
  await expectLater(tester, meetsGuideline(androidTapTargetGuideline));  // skips edge tiles!
});
```

Plus the overflow matrix (30 tests), which asserts `tester.takeException()` is `null` — **overflow already fails a widget test by default** (`DebugOverflowIndicatorMixin._reportOverflow` → `FlutterError.reportError` → the binding rethrows at test end). The entire blog genre on this topic is about how to **suppress** it. So: **never** call `takeException()` to swallow, and **never** set `FlutterError.onError` in a layout test.

`TextScaler.linear(2.0)` is a **deliberate over-approximation** — Android 14+ scales nonlinearly (large text scales *less*). It stresses big tile labels harder than a real device, which is the right conservatism. Document that, and test 1.3/1.5 too, precisely because nonlinear scaling makes the mid-range non-obvious.

### 5.3 What automation genuinely cannot cover

Deque's much-cited **57%** figure is for **axe-core's ~100 web rules**. Flutter ships **four** guidelines — roughly the "trivially machine-checkable" subset — and one is known-broken. Realistic expectation: a small minority of real issues.

**Switch Access / Switch Control cannot be tested automatically at all.** No API simulates scanning, group selection, or point scanning. Traversal order is the only proxy, and it's a *weak* one: Point Scan is coordinate-based (no order), Group Selection is a nested binary narrowing, and scanning targets *actionable* elements while semantics traversal enumerates non-actionable nodes too. Pair the semantics-order test with a `Tab`-key `FocusTraversalPolicy` test — but treat both as regression guards on *intent*, not as conformance evidence.

Also useless here: **Espresso `AccessibilityChecks`** — it walks the Android **View** hierarchy, and Flutter is one opaque `FlutterView`. Google's **Accessibility Scanner** *does* work (it's an `AccessibilityService` and reads Flutter's `AccessibilityBridge` virtual node tree) — but it's manual, on-device, human-driven. And flutter#111110 means the semantics tree isn't even exposed during `flutter drive` unless an accessibility service is pre-enabled. Don't build CI a11y automation.

### 5.4 `docs/a11y-manual-checklist.md`

Ship this **in the repo**, not a wiki. It is the single most valuable a11y artifact for the exit plan: it tells a stranger what "accessible" means for this app and how to verify it without your devices or your knowledge.

```markdown
# Accessibility release checklist — MANUAL, required before every tag

Automated tests catch a minority of a11y issues. Switch Control / Switch Access
have NO automation. There is no telemetry: if this is wrong in the field, we will
never find out. This pass IS the safety net.

Device: ______  OS: ______  Date: ______  Tester: ______
...
- [ ] Personal Voice (if configured) is selectable and speaks
- [ ] Personal Voice permission DENIED -> graceful fallback, never silence
...
- [ ] Music playing -> tile speech DUCKS the music; music resumes after
- [ ] AIRPLANE MODE ON, tap every tile -> all speak
      (catches a network_required voice slipping through voice_filter)
...
## Native surfaces — zero Dart coverage by design
- [ ] QS tile: add to shade. FORCE-STOP the app. Tap tile -> speaks.
- [ ] QS tile: edit the phrase in-app, force-stop, tap tile -> speaks the NEW phrase
      (catches the storage-contract break)
- [ ] QS tile with screen LOCKED -> speaks, or prompts unlock predictably
- [ ] iOS ControlWidget: same three checks
...
## Crash log — the only field signal that will ever exist
- [ ] Trigger a known crash in a debug build; export the log
- [ ] Stack trace has READABLE Dart function names. Hex offsets mean --obfuscate
      or --split-debug-info crept in and the only field signal is dead.

## Scanners
- [ ] Google Accessibility Scanner on the grid screen — no new findings
      (it works: reads Flutter's AccessibilityBridge virtual nodes)
- [ ] Xcode Accessibility Inspector audit on the grid screen — no new findings
```

---

## 6. Highest-value and most over-rated

### Highest value: `test/speech/silence_is_impossible_test.dart`

One parameterized test looping every `SpeechFailure` mode, asserting each yields **audible speech OR a visible on-screen fallback — never neither**.

It wins because it's the only test in the suite that is **structurally unsatisfiable by a code path that fails silently**. Every other test asserts a specific behavior; this one asserts the *absence of a whole failure class*, and adding a value to the enum forces the UI to handle it or the build goes red. It is the closest thing to telemetry this app will ever have, because it turns "I thought of a new way it can go silent" into a compiler-adjacent obligation rather than a note in a backlog.

**Runners-up:** the migration data-survival test (§4.3) — nothing else stands between a v1→v3 user and an empty board. And the pre-migration backup, which is *not a test at all* and is the highest safety-per-line item in the project.

### Most over-rated: `meetsGuideline(...)` as an accessibility gate

This is the thing everyone reaches for to say "we test accessibility," and on **this app's geometry** it is close to fraudulent:

- **`androidTapTargetGuideline` / `iOSTapTargetGuideline`** — `_isAtBoundary` skips every tappable node flush with the view edge. A full-bleed 3×4 grid → **10 of 12 tiles never checked**. It passes vacuously.
- **`textContrastGuideline`** — flutter#103235, open: white on `#fafafa` passes. It also only sees text findable via `find.text`, so a `CustomPainter` label is invisible to it.
- **`labeledTapTargetGuideline`** — the only genuinely load-bearing one, and it merely checks a label is *non-empty*. `button1` passes. A tile leaking its entire vocalization passes.

Four green `expectLater`s buy roughly one real property, and produce a strong false sense that "accessibility is tested." Keep them — they're one line each and catch catastrophic regressions — but **the gates are `getSize` over all 12 tiles, per-tile `isSemantics` label assertions, a pure-Dart contrast unit test, and the manual checklist.**

**Runners-up:** a **coverage percentage gate** (`--coverage` omits untested files, so the number lies *upward* — worse than none), and **integration/emulator tests** (15+ min, flaky, and the emulator has no TTS engine, so the one thing that matters is structurally unreachable).

---

## 7. The uncomfortable summary

Four of the highest-severity failure modes in this app — **`.ambient` audio session muting on the silent switch**, **engine reports success but emits no audio**, **the QS tile speaking a stale phrase**, and **Switch Access focus traps** — are **not testable by any automated means available in 2026**.

That is not a gap to be closed with more tests. It is a gap closed by:

1. **The manual device checklist, committed and run before every tag** — a deliverable, not a chore.
2. **Making failure loud in-app**, because the user is now the only reporting channel.
3. **The on-device exportable crash log**, tested harder than the tiles, with **no `--obfuscate` and no `--split-debug-info`** — obfuscation protects nothing once the source is public, and after abandonment nobody will hold the per-build `.symbols` file, making every crash report ever filed permanently unreadable.
4. **ADRs**, because the decisions that prevent these failures all *look like mistakes*: a nullable FK inside a composite PK reads as a normalization error; `.playback` reads as an oversight; checking a `setVoice` return reads as paranoia; Riverpod for 12 tiles reads as over-engineering. A stranger will "clean up" every one of them, reintroduce the exact failure it prevents, and **no test and no crash report will tell them.** ADRs are ~20 minutes each and are the only durable defense — the one piece of team ceremony worth *more* to a solo dev than to a team, precisely because of the abandonment plan.
...
Target 300-500 lines. Show right-vs-wrong code pairs — they teach faster than prose.
````
